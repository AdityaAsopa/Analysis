...
class EvalFunc:
    ...
    def fitCell( self ):
        ...
        colNames = ['p0', 'p1', 'p2','p3', 'p4','p5', 'p6','p7']
        freqList = self.celldf['freq'].unique()
        for ff in freqList:
            meanSTP = self.celldf.loc[
                    (self.celldf['freq']==ff) & 
                    (self.celldf['exc']==self.exc) & 
                    (self.celldf['numSq']==self.numSq) &
                    (self.celldf['pattern']==self.pattern)
                ][colNames].mean(axis=0)
            self.dumpFindSim( meanSTP, ff, True, False ) # for the peaks
            if self.doPairedPulse:
                self.dumpFindSim( meanSTP, ff, True, True ) # for the paired pulse
            if ( self.matchMinima ):
                self.dumpFindSim( meanSTP, ff, False, False ) # for the minima
    ...
    def dumpFindSim( self, deconv, freq, isPk, isPP ):
        fileSig = "{}_{}_{}_{}".format( self.cell, self.exc, self.numSq, self.pattern )
        stimAmpl = 50.0
        Ca_basal = 0.08
        stimWidth = 0.002
    ...
    "Readouts": {{
        ...
        with open( fname, "w" ) as fp:
            ...
            for idx in range( len( deconv ) ):
                if isPP and idx > 1 :   # Only do first 2 points
                    continue
                fp.write( ",\n                [{:.4f}, {:.3f}],\n".format( settleTime + idx/freq, stimAmpl ) )
                fp.write( "                [{:.4f}, {:.3f}]".format( stimWidth + settleTime + idx/freq, Ca_basal ) )
            fp.write( fsMidStr )
            comma = ""
            for idx, dd in enumerate( deconv ):
                ...
                if isPk:
                    fp.write( "{}\n            [{:.4f}, {:.4f}, 0]".format( 
                        comma, pd[idx] + settleTime + idx/freq, 
                        fracVesicleRelease * dd/deconv[0] ) )
                else:
                    fp.write( "{}\n            [{:.4f}, {:.4f}, 0]".format( 
                        comma, transmitterDecayTime + pd[idx] + settleTime + idx/freq, 
                        transmitterBaseline ) )
                comma = ","
            fp.write( fsEndStr )
...
def main():
    ...
    parser = argparse.ArgumentParser( description = "Fits presynaptic release model to STP curves from pandas file" )
    ...
    parser.add_argument( "-f", "--file", type = str, 
            help = "Optional: Name of tabulated STP file in pandas hdf5 format. Default = 'stp.h5'", 
            default = "stp.h5" )
